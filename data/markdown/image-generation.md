...
## ⚠️ 在线酒馆（本站）用户必读:为什么填 127.0.0.1 连不上

如果你用的是 **在线云酒馆**（浏览器打开直接用，不是自己电脑上装的），
那么填写 `http://127.0.0.1:7860` **一定会失败**，报 500 错误。

### 原因

图像生成的请求**不是由你的浏览器发出的**，而是由云酒馆服务器发出的：

```
你的浏览器  →  云服务器(运行SillyTavern)  →  去访问你填的地址
```

当你填 `127.0.0.1:7860` 时，服务器会去访问**它自己的 7860 端口**（那里什么都没有），
而不是你电脑上的 SD WebUI。所以必然连接失败。

同样的原因，以下功能在**在线酒馆**里都无法直连本地服务:

- 本地 Stable Diffusion / ComfyUI（画图）
- 本地 KoboldAI / text-generation-webui / Ollama（本地大模型）
- 本地 TTS 语音服务

### 解决方案（三选一）

#### 方案 A:使用站内内置画图模型（推荐，零配置）

使用AI 画图模型，直接在对话中让 AI 画图即可，不需要任何本地部署:

- 在聊天室的 API 配置里，模型列表中选择带画图能力的模型
- 直接在对话中描述你想要的画面

优点:零配置、无需显卡、无需网络穿透。
缺点:按积分计费，风格受模型限制。

#### 方案 B:把本地 SD 暴露到公网，填公网地址

如果你坚持用自己的 SD WebUI，需要让它能被外部访问:

**第 1 步:SD WebUI 启动参数**

```
--api --listen --gradio-auth 你的用户名:你的密码
```

- `--api` 开启 API（必须）
- `--listen` 让 SD 监听所有网卡（**关键**，默认只监听 127.0.0.1，外部连不上）
- `--gradio-auth` 设置访问密码（**强烈建议**，否则任何人都能白用你的显卡）

**第 2 步:路由器端口映射**

在路由器后台把外部 7860 端口转发到你电脑的内网 IP:7860。
同时确认电脑防火墙允许 7860 入站。

**第 3 步:查询你的公网 IP**

浏览器打开 `https://ip.sb` 或 `https://myip.ipip.net`

**第 4 步:在扩展里填写**

- 服务器 URL: `http://你的公网IP:7860`
- Authentication: `你的用户名:你的密码`（如果设了 gradio-auth）

**可能的障碍:没有公网 IP**

很多家庭宽带是运营商 NAT 共享，没有独立公网 IP。判断方法:
把 `ip.sb` 显示的 IP 和路由器 WAN 口显示的 IP 对比，**不一致就是 NAT**。

这种情况需要:
- 联系宽带运营商申请公网 IP（部分地区免费）
- 或使用内网穿透工具（见方案 C）

⚠️ **安全提醒**:把 SD 暴露到公网有风险。务必设置 `--gradio-auth` 密码，
建议用完就关闭端口映射。本站不对此方式提供技术支持。

#### 方案 C:内网穿透工具

如果没有公网 IP，可以用内网穿透把本地服务映射到一个公网域名:

- cpolar、ngrok、frp、Tailscale Funnel 等（自行搜索配置教程）
- 穿透后会得到一个类似 `https://xxxx.ngrok-free.app` 的地址
- 在扩展的服务器 URL 里填这个地址

同样建议配合 `--gradio-auth` 设置密码。

#### 方案 D:在自己电脑上本地部署 SillyTavern

如果你需要深度使用本地 SD、本地模型、本地 TTS 等功能，
**本地部署的 SillyTavern 才是最合适的选择**——因为它和 SD 跑在同一台电脑上，
填 `127.0.0.1:7860` 就能直接连通。

本地部署教程: [本地安装指南](/getting-started/local-install/)

### 一句话总结

| 你用的是 | 填什么地址 |
|---------|-----------|
| 在线酒馆 + 本地SD | 填**公网IP**或**穿透域名**，不能填127.0.0.1 |
| 在线酒馆 + AI画图模型 | 无需配置，直接用 |
| 自己电脑本地部署的酒馆 | 填 `http://127.0.0.1:7860` 即可 |
